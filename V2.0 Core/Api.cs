    ...
    public class API
    {
        ...
        private  dynamic Auth_call(string resource, string method, string content)
        {
            ...
            try
            {
                ...
                {
                    httpClient.Timeout = new TimeSpan(3, 50, 0);

                    httpClient.DefaultRequestHeaders.Add("api-key", accessId);


                    if (method=="PUT" || method=="POST")
                    {
                        StringContent queryString = new StringContent(content);
                        using (var response = httpClient.PostAsync(url, queryString).Result)
                        {
        ...
        */
        public dynamic send_bat_sms(Dictionary<string, Object> data)
        {
            ...
            string mobilephone = data["to"].ToString();
             var phone = System.Net.WebUtility.HtmlDecode(mobilephone);
